...
def main(args):
    ...
    f = open(
        f'run_logs_re_clustream/{args.ds}_{args.method}_{args.offline}_{args.sumlimit}_{args.gennum}_{args.gpu}_{args.category}_{args.startindex}.txt',
        'w',
        newline='\n', buffering=100)
    offline_dicts = {}
    ...
    elif args.category == "km":
        offlinemethods.append("kmeans")
        offlinemethods.append("wkmeans")
        offlinemethods.append("subkmeans")
    elif args.category == "sc":
        offlinemethods.append("spectral")
        offlinemethods.append("spectacl")
        offlinemethods.append("scar")
    ...
    for param_dict in param_dicts:
        ...
        for i in range(len(X)):
            ...
            if ((i + 1) % args.offline == 0 and i > 0) or is_last:
                ...
                #gen_data = np.load(f"./gen_data/data_{args.ds}_wclustream_{args.offline}_{args.sumlimit}_{args.gennum}_False_{param_index}_{i+1}.npy",
                #               allow_pickle=True)

                mcs = np.load(f"./mc_data/mcs_{args.ds}_clustream_1000_100_1000_False_{param_index}_{i+1}.npy",
                    allow_pickle = True)
                gen_data = []
                weight_data = []
                for mc in mcs:
                    center = dict_to_np(mc[1])
                    weight = int(mc[3])
                    weight_data.append(weight)
                    gen_data.append(center)
                #print(i, len(gen_data), flush=True)
                gen_data_step[i] = gen_data
                weight_data_step[i] = weight_data
            ...
            for alg in offline_dict.keys():
                ...
                for alg_dict in offline_dict[alg]:
                    ...
                    for step in steps:
                        ...
                        cur_weight_data = weight_data_step[step]
                        print(cur_gen_data)
                        try:
                            if alg != "wkmeans":
                                ...
                                clustering = _kmeans.fit_predict(cur_gen_data,
                                                                 sample_weight=cur_weight_data)
                        except:
