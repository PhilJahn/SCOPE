...
def main(args):
    ...
    parser.add_argument('--category', default="means", type=str, help='Offline algorithm category')
    ...
    f = open(
        f'run_logs_re_scope/{args.ds}_{args.method}_{args.offline}_{args.sumlimit}_{args.gennum}_{args.gpu}_{args.category}_{args.startindex}.txt',
        'w',
        newline='\n', buffering=100)
    offline_dicts = {}
    param_dicts = utils.load_parameters(args.ds, args.method, args.used_full)
    offlinemethods = []
    if args.category == "nkest":
    ...
    for param_dict in param_dicts:
        ...
        for i in range(len(X)):
            ...
            if ((i + 1) % args.offline == 0 and i > 0) or is_last:
                ...
                gen_data = np.load(f"./gen_data/data_{args.ds}_scope_full_{args.offline}_{args.sumlimit}_{args.gennum}_False_{param_index}_{i+1}.npy",
                               allow_pickle=True)
