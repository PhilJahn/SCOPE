...
As the initial runs of CluStream-W and CluStream-S failed to terminate on KDDCUP99, we used ```reconstructWCluStream.py``` and ```reconstructScaledCluStream.py``` to obtain the results based on the stored micro-clusters/sampled data of the initial experiments. The methods should be equivalent to the ```flex_offline``` part of ```runMethod.py```. (As _get_closest_mc of a finished base run was not available for CluStream-W, it was substituted with an equivalent KD-Tree.) In practice, these were only included for transparency, as the regular ```runMethod.py``` should be preferred and is required before these can be used. They offer the options of ```nkest``` for partitioning approaches, ```density``` for all density-based approaches, ```vardbscan``` for density-connectivity approaches and ```kest``` for X-Means. 
